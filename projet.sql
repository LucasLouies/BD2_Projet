...
--Prof .1
CREATE
OR REPLACE FUNCTION projet.encoder_etudiant(_nom VARCHAR(100), _prenom VARCHAR(100), _mail VARCHAR(50),
                                                   _semestre VARCHAR(2), _mdp VARCHAR(100)) RETURNS VOID AS
$$
DECLARE
BEGIN
INSERT INTO projet.etudiants
VALUES (DEFAULT, _ nom, _ prenom, _ semestre, _ mdp, DEFAULT, _ mail);
END;
$$
LANGUAGE plpgsql;
...
--Prof .2
CREATE
OR REPLACE FUNCTION projet.encoder_entreprise(_identifiant VARCHAR(3), _nom VARCHAR(100), _mdp VARCHAR(100),
...
VALUES (_ identifiant, _ mdp, _ mail, _ adresse, _ nom) RETURNING id_entreprise
INTO id;
RETURN id;
END;
$$
...
--Prof .3
CREATE
OR REPLACE FUNCTION projet.encoder_mot_cle(_libelle VARCHAR(50)) RETURNS INTEGER AS
$$
DECLARE
id INTEGER := 0;
BEGIN
INSERT INTO projet.mots_cle
VALUES (DEFAULT, _ libelle) RETURNING code_mot_cle
INTO id;
RETURN id;
END;
$$
...
--Prof .5
CREATE
OR REPLACE FUNCTION projet.valider_offre_stage(_code_offre VARCHAR(5)) RETURNS VARCHAR(5) AS
$$
DECLARE
...
SET etat = ''Validée''
WHERE code_offre_stage = _code_offre;
RETURN
_
code_offre;
END;
...
--Entreprise .1
CREATE
OR REPLACE FUNCTION projet.encoder_offre_stage(_description VARCHAR(50), _code_entreprise VARCHAR(3),
                                                      _semestre VARCHAR(2)) RETURNS VARCHAR(5) AS
$$
DECLARE
_code_offre_stage   VARCHAR(5);
    _numero_offre_stage VARCHAR
(2);
BEGIN
...
VALUES (_ code_offre_stage, _ description, _ semestre, DEFAULT, NULL, _ code_entreprise);
RETURN
_
code_offre_stage;
END;
...
--Entreprise .3
CREATE
OR REPLACE FUNCTION projet.ajouter_mot_cle(_code_offre_stage VARCHAR(3), _libelle_mot_cle VARCHAR(50)) RETURNS VARCHAR(9) AS
$$
DECLARE
...
INTEGER;
BEGIN
SELECT mc.code_mot_cle
FROM projet.mots_cle mc
WHERE mc.libelle = _libelle_mot_cle INTO temp_code_mot_cle;
concatenated_value
:= _libelle_mot_cle || ''_'' || _code_offre_stage;
INSERT INTO projet.mots_cle_stage(code_mot_cle, code_offre_stage)
VALUES (temp_code_mot_cle, _ code_offre_stage);
...
encore été attribuée, il sera indiqué "pas attribuée" à la place du nom de l''étudiant.
  */
CREATE
OR REPLACE VIEW projet.voir_offres_stage AS
...
FROM projet.offres_stage os
    LEFT JOIN
    projet.etudiants e
ON os.id_etudiant = e.id_etudiant;
...
CREATE
OR REPLACE FUNCTION projet.voir_candidatures_par_entreprise(_id_entreprise VARCHAR(3))
    RETURNS TABLE
            (
...
BEGIN
    IF
NOT EXISTS(SELECT 1 FROM projet.offres_stage WHERE id_entreprise = _id_entreprise) THEN
...
--Entreprise .6
CREATE
OR REPLACE FUNCTION projet.selectionner_etudiant(_code_stage VARCHAR(3), _mail_etudiant VARCHAR(100)) RETURNS INTEGER AS
$$
...
UPDATE projet.offres_stage
SET etat = ''Attribuée''
WHERE code_offre_stage = _code_stage;

SELECT e.id_etudiant
FROM projet.etudiants e
WHERE e.mail = _mail_etudiant INTO id_candidat;
UPDATE projet.candidatures
SET etat = ''Acceptée''
WHERE id_etudiant = id_candidat;
RETURN id_candidat;
END;
$$
LANGUAGE plpgsql;
...
--Entreprise .7
CREATE
OR REPLACE FUNCTION projet.annuler_stage(_code_stage VARCHAR(3)) RETURNS VOID AS
$$
DECLARE
BEGIN
UPDATE projet.offres_stage
...
une même ligne).
  */
CREATE
OR REPLACE FUNCTION projet.get_offres_stage_valides(_etudiant_mail VARCHAR(50))
    RETURNS TABLE
            (
                code_offre_stage   VARCHAR(5),
...
SELECT e.id_etudiant
FROM projet.etudiants e
WHERE e.mail = _etudiant_mail INTO temp_id_etudiant;
RETURN QUERY
...
*/
CREATE
OR REPLACE FUNCTION projet.rechercher_offres_par_mot_cle(_etudiant_mail VARCHAR(50), _mot_cle VARCHAR(50))
    RETURNS TABLE
            (
                code_offre_stage   VARCHAR(5),
                nom_entreprise     VARCHAR(100),
...
SELECT e.id_etudiant
FROM projet.etudiants e
WHERE e.mail = _etudiant_mail INTO temp_etudiant_id;
RETURN QUERY
SELECT *
...
SELECT e.id_etudiant
FROM projet.etudiants e
WHERE e.mail = _mail_etudiant INTO _id_etudiant;
INSERT INTO projet.candidatures (etat, motivation, code_offre_stage, id_etudiant)
VALUES (''En attente'', _ motivation, _ code_offre_stage, _ id_etudiant) RETURNING id_candidature
INTO _id_candidature;
...
CREATE
OR REPLACE FUNCTION projet.annuler_candidature(_mail_etudiant VARCHAR(50) ,_code_offre_stage VARCHAR(5)) RETURNS VARCHAR(5) AS --fixme décrémenter nb candidature en attente
$$
DECLARE
_id_etudiant INTEGER;
BEGIN
...
CREATE
OR REPLACE FUNCTION projet.vérification_etat_offre_stage() RETURNS TRIGGER AS
$$
DECLARE

BEGIN
    IF
(OLD.etat = ''Non Validée'' AND NEW.etat = ''Attribuée'') OR
...
CREATE
OR REPLACE FUNCTION projet.vérification_etat_candidature() RETURNS TRIGGER AS
$$
DECLARE

BEGIN
    IF
((old.etat = ''Annulée'' OR old.etat = ''Acceptée'' OR old.etat = ''Refusée'') AND old.etat != new.etat)
    THEN
...
CREATE
OR REPLACE FUNCTION projet.vérification_insert_offre_stage() RETURNS TRIGGER AS
$$
DECLARE
...
CREATE
OR REPLACE FUNCTION projet.vérification_insert_stage_mot_cle() RETURNS TRIGGER AS
$$
DECLARE
...
SELECT os.etat
FROM projet.offres_stage os
WHERE NEW.code_offre_stage = os.code_offre_stage INTO etat_stage;
IF
(etat_stage = ''Attribuée'' OR etat_stage = ''Annulée'')
    THEN
...
CREATE
OR REPLACE FUNCTION projet.annulation_validation_stage() RETURNS TRIGGER AS
$$
DECLARE

BEGIN
    IF
((OLD.etat = ''Validée'' AND NEW.etat = ''Annulée''))
    THEN
UPDATE projet.candidatures
SET etat = ''Refusée''
...
CREATE
OR REPLACE FUNCTION projet.verification_poser_candidature() RETURNS TRIGGER AS
$$
DECLARE
...
CREATE
OR REPLACE FUNCTION projet.mettre_a_jour_nb_candidature_en_attente() RETURNS TRIGGER AS
$$
DECLARE
